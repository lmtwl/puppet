...
struct nat_mapping
{
	...
	int refer_count;   /* how many references linked to this mapping * aka. length of original_tuple_list */
...
static char* nf_ct_stringify_tuple(const struct nf_conntrack_tuple *t)
{
	snprintf(tuple_tmp_string, sizeof(tuple_tmp_string), "%pI4:%hu -> %pI4:%hu",
		&t->src.u3.ip, be16_to_cpu(t->src.u.all), &t->dst.u3.ip, be16_to_cpu(t->dst.u.all));
...
static void kill_mapping(struct nat_mapping *mapping)
{
	...
	list_for_each_safe(iter, tmp, &mapping->original_tuple_list)
	{
		...
		if(original_tuple_item == NULL) continue;
...
static int check_mapping(struct nat_mapping* mapping, struct net *net, const struct nf_conntrack_zone *zone)
{
	...
	if(mapping->port == 0 || mapping->int_addr == 0 || mapping->int_port == 0 || mapping->ifindex == -1) return 0;
...
static void handle_dying_tuples(void)
{
	...
	list_for_each_safe(iter, tmp, &dying_tuple_list)
	{
		...
		if(mapping == NULL)
		{
			...
			port = be16_to_cpu((ct_tuple->src).u.udp.port);
			mapping = get_mapping_by_int_src(ip, port);
			if(mapping != NULL) pr_debug("[xt_FULLCONENAT] handle_dying_tuples(): INBOUND dying conntrack at ext port %d\n", mapping->port);
		}
		else
		{
...
static int ct_event_cb(unsigned int events, struct nf_ct_event *item)
{
	...
	if(ct == NULL || !(events & (1 << IPCT_DESTROY))) return 0;
	...
	if(protonum != IPPROTO_UDP) return 0;

	port = cpu_to_be16(ct_tuple_original->dst.u.udp.port);
	if(port < BASE_UDP_PORT) return 0;
	...
	if(wq != NULL) queue_delayed_work(wq, &gc_worker_wk, msecs_to_jiffies(100));
...
static uint16_t find_appropriate_port(struct net *net, const struct nf_conntrack_zone *zone, 
		const uint16_t original_port, const int ifindex, const struct nf_nat_ipv4_range *range)
{
	...
	if((range->flags & NF_NAT_RANGE_PROTO_RANDOM) || (range->flags & NF_NAT_RANGE_PROTO_RANDOM_FULLY))
	{
	...
	else
	{
			...
			|| !(range->flags & NF_NAT_RANGE_PROTO_SPECIFIED))
		{
			...
			if(mapping == NULL || !(check_mapping(mapping, net, zone))) return original_port;
		}
	...
	for (i = 0; i < range_size; i++)
	{
		...
		if(mapping == NULL || !(check_mapping(mapping, net, zone))) return selected;
	}
...
static unsigned int fullconenat_tg(struct sk_buff *skb, const struct xt_action_param *par)
{
	...
	if(xt_hooknum(par) == NF_INET_PRE_ROUTING)
	{
		...
		if(port < BASE_UDP_PORT) return ret;
		...
		* in case the packet needs to be forwarded from another inbound interface. */
		...
		if(check_mapping(mapping, net, zone))
		{
			...
			if(ret == NF_ACCEPT)
			{
				...
				pr_debug("[xt_FULLCONENAT] fullconenat(): INBOUND: refer_count for mapping at ext_port %d is now %d\n", mapping->port, mapping->refer_count);
			}
		}
		spin_unlock_bh(&fullconenat_lock);
		return ret;
	...
	else if(xt_hooknum(par) == NF_INET_POST_ROUTING)
	{
		...
		if(original_port < BASE_UDP_PORT) return ret;
		...
		if(nat != NULL) nat->masq_index = ifindex;
		...
		{
			add_original_tuple_to_mapping(mapping, ct_tuple_origin);
			pr_debug("[xt_FULLCONENAT] fullconenat(): OUTBOUND: refer_count for mapping at ext_port %d is now %d\n", mapping->port, mapping->refer_count);
		}
...
static int fullconenat_check(const struct xt_tgchk_param *par)
{
	mutex_lock(&nf_ct_net_event_lock);

	tg_refer_count++;

	pr_debug("[xt_FULLCONENAT] fullconenat_check(): tg_refer_count is now %d\n", tg_refer_count);
	...
	if(tg_refer_count == 1) 
	{
		...
		if(nf_conntrack_register_notifier(par->net, &ct_event_notifier) == 0) 
		{
			...
			pr_debug("[xt_FULLCONENAT] fullconenat_check(): ct_event_notifier registered.\n");
		}
		else
		{
			printk("[xt_FULLCONENAT] warning: failed to register a conntrack notifier.\n");
		}
	}
...
static void fullconenat_destroy(const struct xt_tgdtor_param *par)
{
	mutex_lock(&nf_ct_net_event_lock);

	tg_refer_count--;

	pr_debug("[xt_FULLCONENAT] fullconenat_destroy(): tg_refer_count is now %d\n", tg_refer_count);
	...
	if(tg_refer_count == 0)
	{
		if(ct_event_notifier_registered)
		{
			...
			pr_debug("[xt_FULLCONENAT] fullconenat_destroy(): ct_event_notifier unregistered.\n");
		}
...
static struct xt_target tg_reg[] __read_mostly = {
	{
		...
		.proto      = IPPROTO_UDP,
		.checkentry = fullconenat_check,
		.destroy    = fullconenat_destroy,
		.me         = THIS_MODULE,
	},
};

static int __init fullconenat_init(void)
{
	/*create kernel work queue*/
	wq = create_singlethread_workqueue("xt_FULLCONENAT");
	if(wq == NULL)
	{
		printk("[xt_FULLCONENAT] failed to create workqueue.\n");
		return -1;
	}
	return xt_register_targets(tg_reg, ARRAY_SIZE(tg_reg));
}

static void __exit fullconenat_exit(void)
{
...
module_init(fullconenat_init);
module_exit(fullconenat_exit);
